...
public class JConnector extends JPanel {
    ...
    public JConnector(JComponent source, JComponent dest, int lineArrow, Color lineColor) {
        this(source, dest, lineArrow, CONNECT_LINE_TYPE_SIMPLE, lineColor);
    }
    ...
    protected void calculateLine() {
       ...
       /* rSource.x += this.source.getParent().getBounds().x;
        rSource.y += this.source.getParent().getBounds().y;
        rDest.x += this.dest.getParent().getBounds().x;
        rDest.y += this.dest.getParent().getBounds().y;*/
